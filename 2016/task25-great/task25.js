// 当在同一类对象上有一系列相同的操作时，应使用原型模式，这样会有一系列的共有的方法；
// 在这个例子中  用原型继承的方法中 this指向的是 用构造函数创建的这个对象，而不是DOM对象，应有两个指针 一个指向DOM，一个这个对象本身
// 为了能实现链式操作 应返回this;
// 另外在对ＤＯＭ 进行增删节点时 也要在 TreeNode的childs中进行提现；
// 不可再给一类元素绑定 事件，这样在操作时取不到相应节点；
// 在一系列操作中应先抽象出公共的操作用于多次的调用；
// 在设计时设计好DOM结构，并且类名也设计好；
...
TreeNode.prototype = {
    ...
    // 解耦样式操作，四个参数表示  是否改变 箭头、可见性、改为高亮、改为普通，后两个参数可省略
